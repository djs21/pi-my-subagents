...
- **Resume Tool & Agent Auto-Detection**: `resume.ts:resolveResumeAgent` automatically inspects the target session file's header for the original `agent` name via `session.ts:getSessionAgent`. Fallback chain: explicit `params.agent` → session header `agent` → prefix match → `worker` defaults. `resolveResumeTools` builds the complete allowlist and respects optional `params.tools` override. All subagents (worker, pnr, reviewer, custom) retain their full tools upon resume.
